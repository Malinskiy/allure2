...
class AppLayoutElement extends BaseElement {
  ...
  shouldKeepState(...args: RouteArguments) {
    ...
    return this.options.shouldKeepState
      ? this.options.shouldKeepState(...args)
      : true;
  }
  ...
  renderElement() {
    ...
    try {
      ...
      this.mountChild(
        "content",
        createReportLoadErrorView(error),
        ".app__content",
      );
      return this;
    }
  ...
  onDownloadableClick(e: Event) {
    ...
    const contentType =
      element.dataset.downloadType || "application/octet-stream";
